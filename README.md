...
This method models items based on their textual features (genres, descriptions) and maps user profiles into the exact same vector space. It executes through a advanced three-stage mathematical pipeline:
...
* **Step A: TF-IDF Item Vectorization (Space Modeling)**
  ...
  $$m_{l,p} = \text{TF}(t_p, i_l) \times \log\left(\frac{1 + |I|}{1 + \text{DF}(t_p)}\right)$$

* **Step B: Dynamic User Preference Profiling ($Q_u$)**
  Instead of using static logs, the system dynamically calculates the user vector $Q_u$ (dimension $1 \times o$). It computes the dot product between the user's historical explicit ratings vector $R_u$ and the sub-matrix of items they have already interacted with ($M_{\text{rated}}$):
  $$Q_u = R_u \cdot M_{\text{rated}}$$

* **Step C: Affinity Similarity Prediction ($S_u$)**
  To forecast the user's interest across all items simultaneously, the engine executes a high-speed matrix multiplication. By multiplying the entire corpus matrix $M$ by the transpose of the user profile vector ($Q_u^T$), it extracts a final similarity column matrix $S_u$ (dimension $m \times 1$):
  $$S_u = M Q_u^T = \begin{bmatrix} m_{1,1} & m_{1,2} & \cdots & m_{1,o} \\ m_{2,1} & m_{2,2} & \cdots & m_{2,o} \\ \vdots & \vdots & \ddots & \vdots \\ m_{m,1} & m_{m,2} & \cdots & m_{m,o} \end{bmatrix} \cdot \begin{bmatrix} q_1 \\ q_2 \\ \vdots \\ q_o \end{bmatrix}$$
---
